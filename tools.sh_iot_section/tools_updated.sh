...
display_iot_security_tools_menu() {
    ...
    echo -e "${BYellow}╚══════════════════════════════════════════╝${NC}"
    echo -e "${BCyan}1)${NC} ${White}Aircrack-ng: Crack WEP/WPA-PSK keys using captured data packets${NC}"
    echo -e "${BCyan}2)${NC} ${White}Bettercap: Perform reconnaissance and MITM attacks on IoT and wireless networks${NC}"
    echo -e "${BCyan}3)${NC} ${White}Scapy: Forge, analyze, and manipulate network packets for testing and debugging.${NC}"
    echo -e "${BCyan}4)${NC} ${White}Wifiphisher: Simulate rogue access points for phishing and credential gathering.${NC}"
    echo -e "${BCyan}5)${NC} ${White}Reaver: Perform brute-force attacks on WPS-enabled Wi-Fi networks to recover the WPA/WPA2 passphrase.${NC}"
    echo -e "${BCyan}6)${NC} ${White}Go Back${NC}"
}
...
run_wifiphisher() {
    output_file="${output}_wifiphisher"
...
# Function to update Aircrack-ng
update_aircrack() {
    if ! command -v aircrack-ng &> /dev/null; then
        ...
        git clone https://github.com/aircrack-ng/aircrack-ng.git > /dev/null 2>&1
        cd aircrack-ng
        log_message "Downloading and updating Aircrack-ng..."
        sudo make uninstall > /dev/null 2>&1
        sudo ./build.sh > /dev/null 2>&1
        sudo make install > /dev/null 2>&1
        sudo ldconfig
        log_message "Aircrack-ng updated to the latest version"
    fi
}
...
# Function to install aircrack-ng if not already installed
install_aircrack() {
    if ! command -v aircrack-ng &> /dev/null; then
...
save_vulnerabilities() {
    ...
    case $tool in
        ...
        "aircrack")
            ...
            read -p "Enter the Wi-Fi network's ESSID (optional, press Enter to skip): " essid
            if [[ -n "$essid" ]]; then
                aircrack-ng -w "$wordlist" -e "$essid" "$cap_file" > "$output_file"
            else
                aircrack-ng -w "$wordlist" "$cap_file" > "$output_file"
            fi
        ;;
        "bettercap")
            read -p "Enter the target IP or network (e.g., 192.168.1.0/24): " target
            echo -e "${CYAN}Starting Bettercap...${NC}"
            bettercap -eval "net.probe on; net.show; set net.target $target; attack; sleep 10; exit" > "$output_file"
        ;;
        "scapy")
            read -p "Enter the network interface to capture packets (e.g., eth0): " interface
            read -p "Enter the number of packets to capture: " num_packets
            echo -e "${CYAN}Starting packet capture with Scapy...${NC}"
            sudo python3 -c "
...
with open('$output_file', 'w') as f:
    f.write(str(packets))
" > /dev/null 2>&1
            echo -e "${GREEN}Packet capture completed and saved to $output_file.${NC}"
        ;;
        "wifiphisher")
            echo -e "${CYAN}Running Wifiphisher...${NC}"
            read -p "Enter the target network ESSID: " essid
            wifiphisher --essid "$essid" --logging > "$output_file"
            if [ $? -eq 0 ]; then
                echo -e "${GREEN}Wifiphisher output saved to $output_file.${NC}"
            else
                echo -e "${RED}Failed to run Wifiphisher.${NC}"
                return 1
            fi
        ;;
        ...
        "reaver")
            read -p "Enter the path to the .cap file: " cap_file
            read -p "Enter the Wi-Fi network's ESSID: " essid
            echo -e "${CYAN}Starting Reaver...${NC}"
            reaver -i "$interface" -b "$essid" -w "$wordlist" -vv > "$output_file"
            if [ $? -eq 0 ]; then
                echo -e "${GREEN}Reaver output saved to $output_file.${NC}"
            else
                echo -e "${RED}Failed to run Reaver.${NC}"
                return 1
            fi
        ;;
    ...
    echo -e "${GREEN}Output saved:${NC}"
    cat "$output_file"
    # Prompt user to save the vulnerabilities to a file
    read -p "Do you want to keep the output file? (y/n) " save_to_file
    if [[ "$save_to_file" == "y" ]]; then
        echo -e "${GREEN}Output saved to $output_file.${NC}"
    else
        rm "$output_file"
        echo -e "${GREEN}Output discarded.${NC}"
    fi
}
